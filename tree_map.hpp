...
private:
  ...
  //Fixes red/black properties after removal
  ...
  //Recursively clears the TreeMap
  ...
  //Helper function to collect all node data in consistent format
...
public:
  ...
  std::vector<std::tuple<K, V, std::string, std::string, std::string, std::string, std::string>> toVector() const {
    std::vector<std::tuple<K, V, std::string, std::string, std::string, std::string, std::string>> out;
    std::function<void(TreeNode<K, V>*)> inorder = [&](TreeNode<K, V>* node) {
      if (!node) return;
      inorder(node->left);
      std::string parentAddr = node->parent ? node->parent->address : "nullptr";
      std::string leftAddr = node->left ? node->left->address : "nullptr";
      ...
      out.emplace_back(node->key, node->value, node->address, parentAddr, leftAddr, rightAddr, colorStr);
      inorder(node->right);
    };
    inorder(root);
    return out;
